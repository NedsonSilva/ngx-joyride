...
describe('JoyrideStepsContainerService', () => {
    ...
    describe('getStepsCount', () => {
        it('should return the number of steps passed by optionService', () => {
            joyrideOptionsService.getStepsOrder.and.returnValue([
                'one',
                'two',
                'three'
            ]);
    ...
    describe('updatePosition', () => {
        ...
        it('should throw an error if we try to update a stop that does not exist in the step list', () => {
            ...
            expect(() =>
                joyrideStepsContainerService.updatePosition(
    ...
    describe('get', () => {
        ...
        describe('when init has been called', () => {
            ...
            it('should retrieve the steps when StepActionType.PREV is used', () => {
                ...
                expect(
                    joyrideStepsContainerService.get(StepActionType.PREV)
                ).toEqual(step1);
            });
    ...
    describe('addStep', () => {
        ...
        it("should add one step to the temp list if a step with the same name doesn't exist", () => {
            ...
            expect(joyrideStepsContainerService['tempSteps'][3]).toEqual(
                jasmine.objectContaining(STEP)
            );
        });
    });
    ...
    describe('getStepNumber', () => {
        it('should return the stepPosition + 1', () => {
            ...
            expect(joyrideStepsContainerService.getStepNumber(STEP1.name)).toBe(
                1
            );
            expect(joyrideStepsContainerService.getStepNumber(STEP2.name)).toBe(
                2
            );
            expect(joyrideStepsContainerService.getStepNumber(STEP3.name)).toBe(
                3
            );
        });
    ...
    describe('getStepRoute', () => {
        ...
        it('should return the route of the first step if called twice', () => {
            ...
            expect(
                joyrideStepsContainerService.getStepRoute(StepActionType.NEXT)
            ).toBe('url1');
            expect(
                joyrideStepsContainerService.getStepRoute(StepActionType.NEXT)
            ).toBe('url1');
        ...
        it('should return an empty string if the first call is PREV', () => {
            ...
            expect(
                joyrideStepsContainerService.getStepRoute(StepActionType.PREV)
            ).toBe('');
        });
    });
});
