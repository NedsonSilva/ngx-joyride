...
} from '../../services/joyride-options.service';
...
describe('JoyrideStepComponent', () => {
    ...
    beforeEach(waitForAsync(() => {
        TestBed.configureTestingModule({
            ...
            providers: [
                {
                    provide: JoyrideStepsContainerService,
                    useClass: JoyrideStepsContainerServiceFake
                },
                {
                    provide: EventListenerService,
                    useClass: EventListenerServiceFake
                },
                { provide: DocumentService, useClass: DocumentServiceFake },
                { provide: LoggerService, useClass: LoggerFake },
                {
                    provide: JoyrideOptionsService,
                    useClass: JoyrideOptionsServiceFake
                },
                {
                    provide: JoyrideStepService,
                    useClass: JoyrideStepFakeService
                },
    ...
    beforeEach(() => {
        ...
        childComponent = hostFixture.debugElement.query(
    ...
    describe('ngAfterViewInit()', () => {
        ...
        describe('when stepContent is defined', () => {
            ...
            it('should set the stepContainer max-width to CUSTOM_STEP_MAX_WIDTH_VW', () => {
                ...
                let stepContainer = hostFixture.debugElement.query(
                    By.css('.joyride-step__container')
                );
        ...
        describe('when stepContent is not defined', () => {
            ...
            it('should set the stepContainer max-width to STEP_MAX_WIDTH', () => {
                ...
                expect(stepContainer.nativeElement.style.maxWidth).toBe(
                    '400px'
                );
            });
            ...
            it('should set stepContainer dimensions equals to the default ones', () => {
                ...
                expect(stepContainer.nativeElement.style.height).toBe(
                    '165.017px'
                );
            });
            ...
            it('should set stepHolder position equals to absolute if step.isElementOrAncestorFixed returns false', () => {
                ...
                expect(stepHolder.nativeElement.style.position).toBe(
                    'absolute'
                );
            });
            ...
            describe('when the step position is "top"', () => {
                ...
                it('should call stepsContainerService.updatePosition("top") if the step position is "top"', () => {
                    expect(
                        stepsContainerService.updatePosition
                    ).toHaveBeenCalledWith(STEP.name, 'top');
                });

                it('should show the arrow with position "top"', () => {
                    hostFixture.detectChanges();
                    let arrow = hostFixture.debugElement.query(
                        By.css('.joyride-step__arrow')
                    );
            ...
            describe('when the step position is "bottom"', () => {
                ...
                it('should call stepsContainerService.updatePosition("bottom") if the step position is "bottom"', () => {
                    expect(
                        stepsContainerService.updatePosition
                    ).toHaveBeenCalledWith(STEP.name, 'bottom');
                });
            ...
            describe('when the step position is "right"', () => {
                ...
                it('should call stepsContainerService.updatePosition("right") if the step position is "right"', () => {
                    expect(
                        stepsContainerService.updatePosition
                    ).toHaveBeenCalledWith(STEP.name, 'right');
                });
            ...
            describe('when the step position is "left"', () => {
                ...
                it('should call stepsContainerService.updatePosition("left") if the step position is "left"', () => {
                    expect(
                        stepsContainerService.updatePosition
                    ).toHaveBeenCalledWith(STEP.name, 'left');
                });
    ...
    xdescribe('customLabels', () => {
        it('should set the correct prev text in the HTML', () => {
            ...
            const prevText = fixture.debugElement.query(
                By.css('.joyride-step__prev-button')
            );
    ...
    describe('adjust LEFT Position', () => {
        it(`should prevent to draw the step outside setting leftPosition 
            to DEFAULT_DISTANCE_FROM_MARGIN_LEFT when the step position is top`, () => {
            const targetWidth = 50;
            const elemRef = new FakeElementRef(targetWidth, 10);
            ...
            // Set stepWidth
            spyOn<any>(childComponent, 'adjustDimensions').and.returnValue({
                width: 100,
                height: 30
            });
            ...
            expect(childComponent.leftPosition).toBe(
                DEFAULT_DISTANCE_FROM_MARGIN_LEFT
            );
        });
        it(`should prevent to draw the step outside setting leftPosition 
            to DEFAULT_DISTANCE_FROM_MARGIN_LEFT when the step position is bottom`, () => {
            const targetWidth = 50;
            const elemRef = new FakeElementRef(targetWidth, 10);
        ...
        it(`should NOT prevent to draw the step outside setting leftPosition 
            to DEFAULT_DISTANCE_FROM_MARGIN_LEFT when the step position is right`, () => {
            const targetWidth = 50;
            const elemRef = new FakeElementRef(targetWidth, 10);
            let STEP = new JoyrideStep();
            STEP.targetViewContainer = new FakeViewContainerRef(elemRef);
        ...
        it(`should prevent to draw the step outside setting leftPosition 
            to DEFAULT_DISTANCE_FROM_MARGIN_LEFT when the step position is left`, () => {
            const targetWidth = 50;
            const elemRef = new FakeElementRef(targetWidth, 10);
            const STEP = new JoyrideStep();
            STEP.targetViewContainer = new FakeViewContainerRef(elemRef);
    ...
    describe('adjust TOP Position', () => {
        it(`should prevent to draw the step outside setting topPosition 
            to DEFAULT_DISTANCE_FROM_MARGIN_TOP when the step position is right`, () => {
            const targetHeight = 50;
            const elemRef = new FakeElementRef(19, targetHeight);
            let STEP = new JoyrideStep();
            STEP.targetViewContainer = new FakeViewContainerRef(elemRef);
            STEP.position = 'right';
            ...
            expect(childComponent.topPosition).toBe(
                DEFAULT_DISTANCE_FROM_MARGIN_TOP
            );
        });
        it(`should prevent to draw the step outside setting topPosition 
            to DEFAULT_DISTANCE_FROM_MARGIN_TOP when the step position is left`, () => {
            const targetHeight = 50;
            const elemRef = new FakeElementRef(19, targetHeight);
            let STEP = new JoyrideStep();
            STEP.targetViewContainer = new FakeViewContainerRef(elemRef);
        ...
        it(`should prevent to draw the step outside setting topPosition 
            to DEFAULT_DISTANCE_FROM_MARGIN_TOP when the step position is top`, () => {
            const targetHeight = 50;
            const elemRef = new FakeElementRef(19, targetHeight);
            let STEP = new JoyrideStep();
            STEP.targetViewContainer = new FakeViewContainerRef(elemRef);
        ...
        it(`should NOT prevent to draw the step outside setting topPosition 
            to DEFAULT_DISTANCE_FROM_MARGIN_TOP when the step position is bottom`, () => {
            const targetHeight = 50;
            const elemRef = new FakeElementRef(19, targetHeight);
            let STEP = new JoyrideStep();
            STEP.targetViewContainer = new FakeViewContainerRef(elemRef);
            ...
            STEP.isElementOrAncestorFixed = false;
            documentService.getElementAbsoluteTop.and.returnValue(
                targetAbsoluteLeft
            );
            ...
            expect(childComponent.topPosition).toBe(
                targetAbsoluteLeft + targetHeight + DISTANCE_FROM_TARGET
            );
        });
    });
